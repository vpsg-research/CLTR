...
with open('output/normalized_list_sys0.2.pkl', 'rb') as f:
    normalized_list = pickle.load(f)
with open('output/scaled_mirror_list_sys0.2.pkl', 'rb') as f:
    scaled_mirror_list = pickle.load(f)
...
def adjust_q(current_epoch, initial_q, final_q,  total_epochs):
    return initial_q - (initial_q - final_q) * (current_epoch / total_epochs)
...
def train(train_loader, epoch, model, optimizer1, args, criterion):
    ...
    for i, (data, labels, indexes) in enumerate(train_loader):
        ...
        loss1 = normalized_list[epoch]/100 * delta_sigma
        loss2 = scaled_mirror_list[epoch] * gamma_norm
